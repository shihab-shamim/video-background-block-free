...
const applyResponsiveSources = block => {
  ...
  const posterUrl = block.dataset[`poster${device.charAt(0).toUpperCase() + device.slice(1)}`] || block.dataset.posterDesktop || "";
  ...
  if (video) {
    ...
    sources.forEach(source => {
      const nextSrc = source.dataset[deviceKey] || source.dataset.srcDesktop || "";
  ...
  if (iframe) {
    ...
    const url = block.dataset[`${type}Url${device.charAt(0).toUpperCase() + device.slice(1)}`] || block.dataset[`${type}UrlDesktop`] || "";
    const embed = type === "youtube" ? getYouTubeEmbed(url, noCookie) : getVimeoEmbed(url);
    ...
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) {
        ...
        if (video && !prefersReducedMotion()) {
          const isMuted = block.dataset.audioOnClick === "1" || block.dataset.muted === "1";
          video.muted = isMuted;
